// React Bits MorphSlider, supplied by the user. See docs/shader-audit/2026-09-15-morph.
// Sample in landscape UV space; textures keep encoded RGB, matching OGL.
...
uniform vec4 uCurrentCrop;
uniform vec4 uNextCrop;
vec4 readCurrent(vec2 p) { return texture2D(tCurrent, uCurrentCrop.xy + clamp(p, 0.0, 1.0) * uCurrentCrop.zw); }
vec4 readNext(vec2 p) { return texture2D(tNext, uNextCrop.xy + clamp(p, 0.0, 1.0) * uNextCrop.zw); }
...
vec4 sampleMorph(vec2 vUv) {
  ...
  vec3 colC = vec3(
    readCurrent(sC + vec2(ca, 0.0)).r,
    readCurrent(sC).g,
    readCurrent(sC - vec2(ca, 0.0)).b
  );
  vec3 colN = vec3(
    readNext(sN + vec2(ca, 0.0)).r,
    readNext(sN).g,
    readNext(sN - vec2(ca, 0.0)).b
  );
