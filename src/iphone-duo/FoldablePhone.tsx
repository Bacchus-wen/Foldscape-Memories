...
import { FOLD_DURATION, progressToOpeningAngle } from './fold-choreography'
...
export function FoldablePhone({ value, defaultValue = 0, onValueChange, duration = FOLD_DURATION, children, className = '', ...props }: FoldablePhoneProps) {
  ...
  useMotionValueEvent(progress, 'change', latest => {
    ...
    // Controlled animation updates must not echo back into the parent state.
    if (!syncingValue.current) onValueChange?.(latest)
  })
...
export function FoldScrubber({ className = '', ...props }: Omit<ComponentProps<'input'>, 'type' | 'min' | 'max' | 'value' | 'onChange' | 'defaultValue'>) {
  ...
  return <input {...props} className={`duo-scrubber ${className}`} type="range" min={0} max={1} step={0.005} value={value} aria-label={props['aria-label'] ?? 'Fold angle'} aria-valuetext={`${progressToOpeningAngle(value)} degrees open`} onChange={event => setValue(event.currentTarget.valueAsNumber)} />
}
